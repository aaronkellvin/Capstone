"""Build a Word copy of Chapters 4 and 5."""
...
SRC = Path(__file__).resolve().parent / "IT7-Chapter4-5.md"
OUT = Path(__file__).resolve().parent / "IT7-Chapter4-5.docx"
...
def is_table_sep(line: str) -> bool:
    return bool(re.match(r"^\|?\s*:?-{3,}", line.replace("|", " | "))) or bool(
...
def main():
    ...
    while i < len(lines):
        ...
        if stripped.startswith("# ") and not stripped.startswith("##"):
            ...
            add_p(doc, stripped, size=11, italic=False, center=True, after=4, before=10)
        elif stripped.startswith("*Insert:"):
            add_p(doc, stripped.strip("*"), size=10, italic=True, center=True, after=10)
        else:
